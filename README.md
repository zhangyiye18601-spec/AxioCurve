...
AxioCurve 是一个基于 MATLAB App Designer 风格代码实现的科研曲线动画绘图工具。名称来自 axis + curve，强调双轴曲线、科研图形和动画导出。它面向实验数据、流变曲线、多列 CSV 数据、Fluent `.out` 监测输出和论文/PPT 可视化场景，支持静态预览、动画播放、双 y 轴绘图、科研配色、字体与图例样式控制、Fluent `.out` 转 CSV，并可导出 GIF、MP4、PNG 序列以及常见静态图片格式。

> 当前仓库同时提供 MATLAB 源码和 Windows 下载压缩包；仓库只保留可运行软件所需文件，不提交 MATLAB Compiler 中间目录、测试缓存和本地导出文件。
...
- `AxioCurve.exe`：主程序。
- `AxioCurve_RuntimeInstaller_web.exe`：MATLAB Runtime R2024a 联网安装器。
- `README_使用说明.txt`：Windows 版本使用说明。
- `splash.png`：程序启动图片。
...
- Fluent `.out` 转 CSV：在“数据与导出”页中可批量选择 Fluent 导出的 `.out` 文件，并转换为表头与数据列对齐的 `.csv` 文件。
...
- 设置持久化：样式设置、导出设置、图例设置和上次使用的导入/导出路径会在下次打开时恢复；坐标范围会根据当前数据自动适配。
...
1. 在 `数据与导出` 页找到 `Fluent .out 转 CSV` 区域。
2. 点击 `1 选择要转换的文件`，可一次选择一个或多个 `.out` 文件。
3. 点击 `2 选择保存路径及格式转换`，选择 CSV 输出文件夹。
4. 软件会在目标文件夹中生成与源文件同名的 `.csv` 文件。

软件会记住上一次 CSV 导入、图像导出、PNG 序列导出、`.out` 选择和 `.csv` 转换输出路径；下次打开对应对话框时会默认回到上一次使用的文件夹。
...
├── fluentOutToCsv.m     # Fluent .out 转 CSV 的核心转换函数
├── dist/
